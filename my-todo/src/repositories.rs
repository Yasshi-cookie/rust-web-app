...
impl TodoRepository for TodoRepositoryForDb {
    async fn create(&self, payload: CreateTodo) -> anyhow::Result<Todo> {
        ...
        let created_todo = self.find(query_result.last_insert_id() as u16).await?;

        Ok(created_todo)
    }

    async fn find(&self, id: u16) -> anyhow::Result<Todo> {
    ...
    async fn update(&self, id: u16, payload: UpdateTodo) -> anyhow::Result<Todo> {
        ...
        let updated_todo = self.find(id).await?;

        Ok(updated_todo)
    }

    async fn delete(&self, id: u16) -> anyhow::Result<()> {
        sqlx::query(
            r#"
            DELETE FROM
                todos
...
pub mod test_utils {
    ...
    type TodoDatas = HashMap<u16, Todo>;
